...
Define_Module(CSMA802154HoHuT);


void CSMA802154HoHuT::initialize(int stage)
{
	csma::initialize(stage);

	if (stage==1)
	{
	    myMacAddr = par("nodeMacAddr").longValue(); //override with omnetpp.ini par
	    EV << "  host macaddress=" << myMacAddr << endl;
	}
}

cPacket *CSMA802154HoHuT::decapsMsg(MacPkt * macPkt) {
...
void CSMA802154HoHuT::handleLowerControl(cMessage *msg) {
