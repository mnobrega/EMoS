...
Define_Module(WiseRouteHoHuT);

void WiseRouteHoHuT::initialize(int stage)
{
...
WiseRouteHoHuT::~WiseRouteHoHuT()
{
	cancelAndDelete(routeFloodTimer);
}

void WiseRouteHoHuT::handleSelfMsg(cMessage* msg)
{
	if (msg->getKind() == SEND_ROUTE_FLOOD_TIMER) {
		...
		EV << "WiseRouteHoHuT - handleSelfMessage: got unexpected message of kind " << msg->getKind() << endl;
		delete msg;
	}
}


void WiseRouteHoHuT::handleLowerMsg(cMessage* msg)
{
...
void WiseRouteHoHuT::handleLowerControl(cMessage *msg)
{
    delete msg;
}

void WiseRouteHoHuT::handleUpperMsg(cMessage* msg)
{
	...
	if ( cInfo == NULL ) {
	    EV << "WiseRouteHoHuT warning: Application layer did not specifiy a destination L3 address\n"
...
void WiseRouteHoHuT::updateRouteTable(const LAddress::L3Type& origin, const LAddress::L3Type& lastHop, double rssi, double ber)
{
...
cMessage* WiseRouteHoHuT::decapsMsg(WiseRoutePkt *msg)
{
...
WiseRouteHoHuT::floodTypes WiseRouteHoHuT::updateFloodTable(bool isFlood, const tFloodTable::key_type& srcAddr, const tFloodTable::key_type& destAddr, unsigned long seqNum)
{
	if (isFlood) {
